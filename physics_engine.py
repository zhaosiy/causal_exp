...
class BallEngine(Engine):
    ...
    def add_boundary_impulse_int(self, balls, p_range=(-75, 75, -75, 75)):
        f_scale = 5e2
        eps = 2
        for i in range(self.n_ball):
            impulse = np.zeros(2)
            p = np.array([balls[i].position[0], balls[i].position[1]])
            ...
            balls[i].apply_impulse_at_local_point(impulse=impulse, point=(0, 0))
    ...
    def get_int_state(self):
        ...
        for i in range(self.n_ball):
            ball = self.int_balls[i]
            state[i] = np.array([ball.position[0], ball.position[1], ball.velocity[0], ball.velocity[1]])

        vel_dim = self.state_dim // 2
        if self.int_state_prv is None:
            state[:, vel_dim:] = 0
        else:
            state[:, vel_dim:] = (state[:, :vel_dim] - self.int_state_prv[:, :vel_dim]) / self.dt
        return state
    ...
    def apply_intervention(self, bidx, kp_cur, graph_gt, inter_step):
        n_ball = 5
        mass = 1
        radius = 6
        self.intspace = pymunk.Space()
        self.intspace.gravity = (0., 0.)
        self.int_balls = []
        #print(kp_cur.shape,'kpcur')
        edge_type_gt, edge_attr_gt = graph_gt
        idx_gt = torch.argmax(edge_type_gt, dim=-1)
        state_dim = 2
        action_dim = 2
        states_all = np.zeros((inter_step, n_ball, state_dim))
        actions_all = np.zeros((inter_step, n_ball, action_dim))
        # interve with gt graph:
        this_graph = idx_gt[bidx].detach().cpu().numpy()
        this_cur = kp_cur[:,:,:2].detach().cpu().numpy() * 80
        this_attr = edge_attr_gt[bidx].detach().cpu().numpy()
        vxy = (this_cur[-2, : , :2] - this_cur[-1, :, :2])/ self.dt
        # apply intervention:
        # change position and relation length therefore.
        ball_int_pos = np.random.choice(range(n_ball), 3, replace=False)
        # create balls:
        for i in range(n_ball):
            inertia = pymunk.moment_for_circle(mass, 0, radius, (0, 0))
            x = this_cur[-1, i, 0]
            y = this_cur[-1, i, 1]
            if i in ball_int_pos:
                xy_int = np.random.rand(2)
                x = x - xy_int[0] * 20
                y = y - xy_int[1] * 20
            body = pymunk.Body(mass, inertia)
            body.position = Vec2d((x, y))
            #print(vxy[i], vxy.shape)
            body.velocity = vxy[i]
            shape = pymunk.Circle(body, 0., (0, 0))
            shape.elasticity = 1
            self.intspace.add(body, shape)
            self.int_balls.append(body)
        # create relations:
        param_load = this_attr
        for i in range(n_ball):
                for j in range(i):
                    rel_type = this_graph[i, j]
                    pos_i = self.int_balls[i].position
                    pos_j = self.int_balls[j].position
                    ...
                    if rel_type == 0:
                        ...
                        # spring
                        rest_length = param_load[i, j]
                        c = pymunk.DampedSpring(
                            self.int_balls[i], self.int_balls[j], (0, 0), (0, 0),
                            rest_length=rest_length, stiffness=20, damping=0.)
                        self.intspace.add(c)
        ...
        self.int_state_prv = None
        for st in range(inter_step):
            # step
            self.int_state_prv = self.get_int_state()
            act += (np.random.rand(n_ball, 2) - 0.5) * 600 - act * 0.1 #- state[:, 2:] * 0.1
            act = np.clip(act, -1000, 1000)
            for i in range(self.n_ball):
               self.int_balls[i].apply_force_at_local_point(force=act[i], point=(0, 0))
            self.add_boundary_impulse_int(self.int_balls)   
            self.intspace.step(self.dt)
            ...
            vel = state[:, vel_dim:]
            #print(vel,'velocity')
            states_all[st, :, :vel_dim] = pos
            actions_all[st] = act.copy()
        
        if 0:
            kps_cur = kp_cur.cpu().numpy() * 80
            video = False
            image = True
            image_prefix = 'int'
            ...
            for i in range(10):
                ...
                for x in range(n_ball):
                    for y in range(x):
                        rel_type = int(this_graph[x, y]); cnt += 1
                        #print(rel_type,'rel type')
                        if rel_type == 0:
                            continue

                        plt.plot([kps_cur[i, x, 0], kps_cur[i, y, 0]],
                                 [kps_cur[i, x, 1], kps_cur[i, y, 1]],
                                 '-', color=c[rel_type], lw=1, alpha=0.5)
                circles = []
                circles_color = []
                for j in range(n_ball):
                   circle = Circle((kps_cur[i, j, 0], kps_cur[i, j, 1]), radius=self.radius)
                ...
                if image:
                    cv2.imwrite(os.path.join('inter', '%s_%s.png' % ('orig', i)), frame)
            ...
            for i in range(time_step):
                ...
                if image:
                    cv2.imwrite(os.path.join('inter', '%s_%s.png' % (image_prefix, i)), frame)
        ...
        states_all = states_all / 80
        states_all[:, :, 1] *= 1
        actions_all = actions_all / 600
        return states_all, actions_all


    def render_kps(self, kp_cur, graph_gt, kpcurrent):
            bidx = 1
            edge_type_gt, edge_attr_gt = graph_gt
            idx_gt = torch.argmax(edge_type_gt, dim=-1)
            this_graph = idx_gt[bidx].detach().cpu().numpy()
            kps_cur = kp_cur[bidx].cpu().numpy() * 80
            currentkp = kpcurrent[bidx].cpu().numpy() * 80
            video = False
            image = True
            image_prefix = 'int'
            c = ['royalblue', 'tomato', 'limegreen', 'orange', 'violet', 'chocolate', 'black', 'crimson']
            #states = states_all
            #actions = actions_all
            time_step = kps_cur.shape[0]
            cur_tstep = currentkp.shape[0]
            n_ball = kps_cur.shape[1]
            lim=(-80, 80, -80, 80)
            ...
            for i in range(time_step):
                ...
                if image:
                    cv2.imwrite(os.path.join('union', '%s_%s.png' % ('orig', i)), frame)

                plt.close()
            for i in range(cur_tstep):
                fig, ax = plt.subplots(1)
                plt.xlim(lim[0], lim[1])
                plt.ylim(lim[2], lim[3])
                fig.set_size_inches(1.5, 1.5)
                ...
                if image:
                    cv2.imwrite(os.path.join('union', '%s_%s.png' % ('current', i)), frame)
