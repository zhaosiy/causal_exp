...
from model_modules_syn import GRUNet, CNNet, PropNet
...
class interveneTarget(nn.Module):
...
class DynaNetGNN(nn.Module):
    ...
    def __init__(self, args, use_gpu=True, drop_prob=0.2):
        ...
        self.model_dynam_encode = PropNet(
            node_dim_in=args.node_attr_dim + 1,
            edge_dim_in=args.edge_attr_dim + 2,
            nf_hidden=nf,
            node_dim_out=nf,
            edge_dim_out=nf,
            edge_type_num=args.edge_type_num,
        ...
        self.model_dynam_node_forward = GRUNet(
            nf + 1 + args.node_attr_dim + args.action_dim, nf, nf,
            drop_prob=drop_prob)
        self.model_dynam_edge_forward = GRUNet(
            nf + 2  + args.edge_attr_dim + args.action_dim * 2, nf, nf,
            drop_prob=drop_prob)

        self.model_dynam_decode = PropNet(
            node_dim_in=nf + args.node_attr_dim + args.action_dim + 1,
            edge_dim_in=nf + args.edge_attr_dim + args.action_dim * 2 + 2,
            nf_hidden=nf * 2,
            node_dim_out=1,
            edge_dim_out=1,
            edge_type_num=args.edge_type_num,
    ...
    def graph_inference(self, kp, action=None, hard=False, env=None):
        ...
        # print(node_enc.shape,' node enc shape')
        #print(node_enc.view(B * T, n_kp, 1).shape,'what')
        node_rep, edge_rep = self.model_infer_encode(
            node_enc.view(B * T, n_kp, 1), None)
        #print(node_rep.shape, edge_rep.shape, 'rep')
        node_rep = node_rep.view(B, T, n_kp, nf)
        edge_rep = edge_rep.view(B, T, n_kp * n_kp, nf)

        kp_t = kp.transpose(1, 2).contiguous().view(B, n_kp, T, 1)
        ...
        node_rep = torch.cat([
            node_rep, kp_t.view(B * n_kp, T, 1)], 2)
        edge_rep = torch.cat([
            edge_rep, kp_t_r.view(B * n_kp**2, T, 1), kp_t_s.view(B * n_kp**2, T, 1)], 2)
        #print(node_rep.shape, edge_rep.shape,'concat')
    ...
    def dynam_prediction(self, kp, graph, action=None, eps=5e-2, env=None):
        ...
        #print(node_enc.shape, edge_enc.shape,'??')
        node_enc, edge_enc = self.model_dynam_encode(
            node_enc.view(B * n_his, n_kp, 1),
            edge_enc.view(B * n_his, n_kp, n_kp, 3),
        ...
        kp_node = kp.transpose(1, 2).contiguous().view(B, n_kp, n_his, 1)
        ...
        kp_edge = kp_edge.view(B, n_kp**2, n_his, 2)
        ...
        # print(node_enc.shape, edge_enc.shape,'1')
